...
export const NotificationDrawer: React.FC<NotificationDrawerProps> = ({ isOpen, onClose }) => {
  ...
  const { t, tx, tSeverity, tWard, formatTime } = useLanguage();
    ...
    <div className="fixed inset-0 z-50 overflow-hidden animate-fade-in">
      ...
      <div className="fixed inset-y-0 right-0 max-w-md w-full bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-slate-800 shadow-2xl flex flex-col z-50 transition-colors duration-200">
        ...
        <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-sm font-black text-slate-900 dark:text-white font-heading">
                {t('notifications.streamTitle')}
              </h3>
              <p className="text-[10px] text-slate-500 dark:text-slate-400">
                {t('notifications.streamSubtitle')}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              title={soundEnabled ? t('notifications.soundEnabled') : t('notifications.soundMuted')}
              aria-label={soundEnabled ? t('notifications.soundEnabled') : t('notifications.soundMuted')}
            >
        ...
        <div className="px-4 py-2 bg-slate-50 dark:bg-slate-950/60 border-b border-slate-200 dark:border-slate-800/80 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
          <span>{t(notifications.length === 1 ? 'notifications.totalAlerts.one' : 'notifications.totalAlerts.other', { count: notifications.length })}</span>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {notifications.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center p-6 text-slate-400 dark:text-slate-500">
              ...
              <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">{t('notifications.emptyTitle')}</p>
                  ...
                  <div className="flex-1 space-y-1 pr-3">
                    <div className="flex items-center gap-2">
                      <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded border uppercase ${SEVERITY_BG_CLASSES[notif.severity]}`}>
                        {tSeverity(notif.severity)}
                      </span>
                      <span className="text-[10px] text-slate-500 font-medium flex items-center gap-1">
                        <Clock className="w-3 h-3" />
                        {formatTime(notif.timestamp, { hour: '2-digit', minute: '2-digit' })}
                      </span>
                    </div>
                    ...
                    {notif.ward_name && (
                      <div className="text-[10px] text-slate-500 dark:text-slate-400 pt-1 font-medium">
                        📍 {tWard(notif.ward_id, notif.ward_name)} ({t('common.ward')} #{notif.ward_id})
                      </div>
                    )}
                  </div>
                </div>
